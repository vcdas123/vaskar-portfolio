...
const Skills = () => {
  return (
    <div className="flex flex-col gap-[30px] text-center xl:text-left">
      ...
      <p className="max-w-[600px] text-white/60 mx-auto xl:mx-0">{SKILLS_DATA?.desc}</p>
      <ScrollArea className="lg:h-[400px] h-full">
        <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {SKILLS_DATA?.skills?.map((skill: Skill, idx: number) => {
            return (
              <li key={idx} className="bg-[#232329] h-[130px] py-6 px-10 rounded-xl flex justify-center items-center">
                <TooltipProvider>
